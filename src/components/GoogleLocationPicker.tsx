...
import { MapPin, Navigation, Compass, ExternalLink, Radio, LocateFixed } from 'lucide-react';
...
export const GoogleLocationPicker: React.FC<GoogleLocationPickerProps> = ({
  ...
  // If no Google Maps API key is configured, provide a fully interactive OpenStreetMap Leaflet picker!
  if (!apiKey) {
    return (
      <div className="mt-4 rounded-2xl overflow-hidden border border-gray-200 shadow-sm bg-white">
        ...
        <div className="bg-gray-50 px-3.5 py-2.5 border-b border-gray-200 flex flex-wrap items-center justify-between gap-2 text-[11px] font-bold text-gray-600">
          <div className="flex items-center gap-2 text-indigo-600">
            ...
            <span>Click map to place hazard pin (OpenStreetMap)</span>
          </div>
          
          <div className="flex items-center gap-2">
            <span className="font-mono text-gray-500 bg-gray-200/70 px-2 py-0.5 rounded text-[10px]">
        ...
        <div className="relative w-full h-[220px] bg-slate-100">
          ...
          <div className="absolute bottom-2 left-2 z-[400] bg-white/90 backdrop-blur-xs px-2.5 py-1 rounded-lg shadow-xs border border-gray-200 text-[10px] text-gray-600 flex items-center gap-1.5 pointer-events-none">
            ...
            <span>Tap map to adjust pin position</span>
          </div>
        </div>
      </div>
    );
  ...
  // Google Maps implementation if API key is provided
  return (
    <div className="mt-4 rounded-2xl overflow-hidden border border-gray-200 shadow-sm">
      <div className="bg-gray-50 px-4 py-2 border-b border-gray-200 flex items-center justify-between text-[11px] font-bold text-gray-600">
        <span className="flex items-center gap-1.5 text-indigo-600">
          <Navigation className="w-3.5 h-3.5" />
          Click or Drag Pin to Reposition Hazard
        </span>
